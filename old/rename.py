...
if(output == output_mod):
    print "Can't have the source and modified file be the same!!"
    raise;



hfile_input = h5py.File(output,'r')
hfile = h5py.File(output_mod,'w')
#################################
#Cut data
#################################

print "copying and cutting file..."
#hfile_input['magnitude:SDSS_r:rest'].value

mag_r = -2.5*np.log10(hfile_input['diskLuminositiesStellar:SDSS_r:rest'].value + hfile_input['spheroidLuminositiesStellar:SDSS_r:rest'].value)

slct = np.ones(mag_r.size,dtype=bool)
if(use_mr_cut):
    print "using mr cut"
    slct = slct & ( mag_r < mr_cut)
print np.min(mag_r[slct]),np.max(mag_r[slct])
slct_size = np.sum(slct)
i_max =len(hfile_input.keys())
skip_cnt = 0
for i,key in enumerate(hfile_input.keys()):
    print i,'/',i_max
    if('total' in key or "magnitude" in key or ('Line' in key and 'observed' in key) or 'parentIndex'==key):
       #or 'dust' in key or 'Line' in key or 'SED' in key or "LSST" in key or "Contiuum" in key):
        print "\tskipping \t", key
        skip_cnt+=1
        print "\t"+str(skip_cnt)
        continue
