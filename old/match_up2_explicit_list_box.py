...
def pandas_to_hdf5(df,output):
    ...
    hfile.flush()
    hfile.close()
    print "pandas to hdf5", time.time()-start
...
for step in gltcs_steps:
    break; #no need for the light cone
    start = time.time()
...
total_end = time.time()
print "done loading box", total_end - total_start
box_df = pd.concat(dfs,ignore_index=True)
print "done making pd dataframe",time.time() -total_end
print "\t", box_df.keys()
print box_df.head()
print box_df.shape
##Shears are now loaded with the lightcone. No need to match up. 
...
avoids = ["AngularMomentum","hotHalo","basic","blackHoleCount","blackHoleJetPower","blackHoleRadiativeEfficiency","blackHoleSpin","darkMatterProfileScale","indicesBranchTip","position","satellite","siblingIndex","merge","merging","Gas","SED","Dust","dust","age","black","Continuum","Line","Metal","total"]
...
for i in range(0,len(gltcs_file_list)):
    ...
    s2 = time.time()
    print "\t cat",s2-s1
    gal_df = pd.merge(gal_df,box_df,on=["step","nodeIndex"])
    print "gal_df step",gal_df['step'][:].dtype
    s3 = time.time()
    print "\t merg:",s3-s2
