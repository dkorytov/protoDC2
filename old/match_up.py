...
print "mering it all together..."
lc_df = pd.concat(dfs).set_index(['step','nodeIndex'])
...
for i in range(0,gltcs_file_num):
    ...
    dfs.append(pd.DataFrame(gal_dic).set_index(['step','nodeIndex']))
    ...
    lc_df = lc_df.merge(gal_df,how="left",left_index=True,right_index=True)
