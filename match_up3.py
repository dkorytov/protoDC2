...
use_gltcs_file_list = param.get_bool("use_gltcs_file_list")
...
#making a list of galaticus files from a pattern if needed
if not use_gltcs_file_list:
    if "${file_num}" not in gltcs_loc and gltcs_file_num > 1:
        raise ValueError("\"${file_num}\" not in %s"%(gltcs_loc))
    gltcs_file_list = []

    for i in range(0,gltcs_file_num):
        gltcs_file_list.append(gltcs_loc.replace("${file_num}",str(i)))
...
def load_gio(file_loc,step,box=False):
    ...
    if box:
        ...
        dic['redshift'] = np.ones(dic['nodeIndex'].size)*stepz.get_z(step)
...
if(tmp_to_disk):
    ...
    cat_hdf5(tmp_file_loc,tmp_file_num,output_final)
else:
    ...
    pandas_to_hdf5(total_result,output_final)
